<div class="sidebar-menu">
    <ul class="menu">
        ...
        <li class="sidebar-item has-sub {{ request()->is('barang*') ? 'active' : '' }}">
            ...
            <ul class="submenu {{ request()->is('barang*') ? 'submenu-open' : '' }}">
        ...
        <li class="sidebar-item {{ request()->is('notifikasi*') ? 'active' : '' }}">
            <a href="{{ url('/notifikasi') }}" class="sidebar-link">
    ...
    <form id="logout-form-petugas" action="{{ route('logout') }}" method="POST" class="d-none">
        @csrf
    </form>
</div>