...
<head>
    ...
    <title>Master Lokasi Rak - SmartStock</title>
        ...
        <div class="page-heading">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-subtitle text-muted">Kelola titik penyimpanan barang di gudang</p>
                </div>
                <a href="{{ route('lokasi.create') }}" class="btn btn-primary px-4 py-2 shadow-sm fw-bold">
        ...
        <div class="page-content">
            ...
            <div class="card shadow-sm border-0" style="border-radius: 12px;">
                <div class="card-header bg-white border-0 py-3">
                    <form action="{{ route('lokasi.index') }}" method="GET" class="row g-2">
                        <div class="col-md-4">
                            <div class="input-group">
                                <span class="input-group-text bg-light border-0"><i class="fas fa-search text-muted"></i></span>
                                <input type="text" name="search" id="searchInput" class="form-control bg-light border-0" placeholder="Cari nama lokasi atau kode rak..." value="{{ $search }}" autocomplete="off">
                            </div>
                        </div>
                        <div class="col-md-2">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="ps-4 text-muted small">{{ $lokasi->firstItem() + $index }}</td>
                                    <td><span class="fw-bold text-dark">{{ $item->nama_lokasi }}</span></td>
                                    <td><code>{{ $item->kode_rak }}</code></td>
                                    <td><span class="text-muted">{{ Str::limit($item->deskripsi, 50) ?: '-' }}</span></td>
                                    <td class="text-end pe-4">
                                        <div class="btn-group">
                                            <a href="{{ route('lokasi.edit', $item->id_lokasi) }}" class="btn btn-sm btn-outline-warning" title="Edit">
                                ...
                                <tr>
                                    <td colspan="5" class="text-center py-5">
                                        <div class="py-3">
                                            ...
                                            <p class="text-muted">Tidak ada data lokasi ditemukan.</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
                ...
                <div class="card-footer bg-white border-0 py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <p class="small text-muted mb-0">Menampilkan {{ $lokasi->firstItem() ?: 0 }} - {{ $lokasi->lastItem() ?: 0 }} dari {{ $lokasi->total() }} data</p>
                        {{ $lokasi->appends(['search' => $search])->onEachSide(1)->links('pagination::bootstrap-5') }}
                    </div>
                </div>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow" style="border-radius: 12px;">
            ...
            <div class="modal-body py-4">
                ...
                <p class="text-center mb-0">Apakah Anda yakin ingin menghapus lokasi <span id="deleteItemName" class="fw-bold text-danger"></span>?</p>
...
<script>
    $(document).ready(function() {
        $('.btn-delete').on('click', function() {
            ...
            const url = "{{ route('lokasi.destroy', ':id') }}".replace(':id', id);
